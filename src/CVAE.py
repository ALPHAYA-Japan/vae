...
class CVAE:
    ...
    def __init__(self, model_path, data_path = None, is_training = False,
        ...
        # CVAE Parameters
        self.cond_dim       = len(os.listdir(self.train_path))
    ...
    def train_initialization(self, hard_load = True, en_iterations = 1, de_iterations = 1):
        ...
        self.en_input    = tf.placeholder(tf.float32, shape= self.input_shape, name= 'en_input'   )   # Input
        self.labels      = tf.placeholder(tf.float32, shape= self.label_shape, name= 'labels'     )   # Condition vector
    ...
    def encoder(self, x, is_training=True, reuse=False):
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            # Layer 2
            net = layers.linear(net, self.linear_dim >> 1, scope = 'en_fc2',
                                w_initializer = self.w_initializer,
                                b_initializer = self.b_initializer)
    ...
    def latent_space(self, x, is_training=True, reuse=False):
        with tf.variable_scope("latent_space", reuse=reuse):
            ...
            if self.verbose: print(self.z.shape)
    ...
    def concatenate(self, x, y):
        ...
        ones = tf.ones([x.shape[1], x.shape[2], y.shape[3]])
        ones = tf.reshape(ones, [-1, x.shape[1], x.shape[2], y.shape[3]])
        x    = tf.concat([x, y * ones], 3)
        # x    = tf.concat_v2([x, y * ones], 3)
        return x
    ...
    def create_network(self):
        en_input     = self.concatenate(self.en_input, self.labels)
        self.encoded = self.encoder(en_input, is_training = True, reuse = False)

        self.noise_input= self.latent_space(self.encoded, is_training = True, reuse = False)
        self.noise_input= tf.concat([self.noise_input, self.labels], 1)
    ...
    def create_optimizer(self):
        ...
        CE = self.en_input * tf.log(eps + self.decoded) + (1 - self.en_input) * tf.log(eps + 1 - self.decoded)
        CE = tf.reduce_sum(CE, axis = [1,2])
        CE = tf.reduce_sum(CE, axis = [1])

        # KL Divergence loss
        Dkl = 1 + self.z_std - tf.square(self.z_mean) - tf.exp(self.z_std / 2.0)
        Dkl = tf.reduce_sum(Dkl, axis = [1])

        # General Loss
        self.de_loss = tf.reduce_mean(-CE - 0.5 * Dkl)
    ...
    def train(self, max_epoches, show_images = False):
        ...
        for epoch in range(max_epoches):
            for i in range(self.num_batches):
                ...
                if i % 500 == 0:
                    x_v_batch, y_v_batch = self.data.valid.next_batch(self.batch_size)
                    v_loss = self.session.run(self.de_loss,
                                              feed_dict = {self.en_input: x_v_batch,
                                                           self.labels  : y_v_batch})
    ...
    def generate(self):
        ...
        preds  = self.session.run(output, feed_dict={input: x_batch,
                                                     label: y_batch})
    ...
    #
    #................................................................................
    def generate(self, source, destination = None, samples = 20, grid_width=480, grid_height=240, label = 4, max_labels = 10):
        ...
        if os.path.isfile(source):
            ...
            x_batch[0] = utils.add_border(x_batch[0],color = [1.0,0.0,0.0])
            grid  = np.concatenate((x_batch[0], preds[0]), axis=1)
            grid  = (grid * 255.0).astype(np.uint8)
        # For generating a grid of images from a directory of images
            ...
            preds = self.session.run(output, feed_dict = {input: x_batch,
                                                          label: y_batch})
            for i in range(samples):
                x_batch[i] = utils.add_border(x_batch[i],color = [1.0,0.0,0.0])
            grid = self.construct_image_grid(x_batch,preds,samples,grid_width,grid_height)
        else:
